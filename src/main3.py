...
def cnn_network(input_x, mode):
    ...
    with tf.variable_scope("layer1", reuse=tf.AUTO_REUSE):
        ...
        # [b,16,16,64]
        pool1 = _pool(activ1, ksize=[1,3,3,1], strides=[1,2,2,1])		# -> [16*16*64]
        tf.summary.histogram('layer1', pool1)

    with tf.variable_scope("layer2", reuse=tf.AUTO_REUSE):
        # [b,16,16,64]
        filters2 = _weighted_variable([3,3,64,128])
        conv2 = _conv2d(pool1, filters2, [1,1,1,1])		# -> [16*16*128]
        bias2 = _bias_variable([128])
        activ2 = _activation(conv2, bias2)
        # [b,16,16,64]
        pool2 = _pool(activ2, ksize=[1,3,3,1], strides=[1,2,2,1])		# -> [8*8*128]
    ...
    with tf.variable_scope("fc1", reuse=tf.AUTO_REUSE):
        ...
        drop1 = tf.nn.dropout(activ_fc1, keep_prob=0.5) #keep_prob usually 0.5 or 0.3

    with tf.variable_scope("output_layer", reuse=tf.AUTO_REUSE):
        weight5 = _weighted_variable([1024, NUM_CLASS])
        bias5 = _bias_variable([NUM_CLASS])
        softmax = tf.nn.softmax(tf.matmul(drop1, weight5) + bias5)
